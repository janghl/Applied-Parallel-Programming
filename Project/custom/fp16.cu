...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
    ...
    */
    ...
    int b = blockIdx.y;
    int m = blockIdx.z;
    int Wcount = (W_out+31)/32;
    int h = blockIdx.x/Wcount*32 + threadIdx.y;
    int w = blockIdx.x%Wcount*32 + threadIdx.x;
    float temp = 0.0f;
        ...
        for(int c = 0; c < C; c++) // sum over all input channels
        {
            for(int p = 0; p < K; p++) // KxK filter
                for(int q = 0; q < K; q++){
                    ...
                    sum = __hadd(sum, __hmul(__float2half(in_4d(b, c, h*S + p, w*S + q )),__float2half( mask_4d(m, c, p, q))));
                    temp += __half2float(sum);
                }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    const int H_out = (H - K)/S + 1;
    const int W_out = (W - K)/S + 1;
    dim3 dimGrid(ceil(1.0*H_out/32)*ceil(1.0*W_out/32), B, M);
